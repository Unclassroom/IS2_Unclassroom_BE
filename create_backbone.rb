...
rails g migration specific_schedule date:date begin_at:time end_at:time -s
rails g migration cyclic_schedule day:integer begin_at:time end_at:time -s

## DEPENDENT TABLES

rails g migration building name:string head_building:references faculty:references -s
rails g migration department faculty:references name:string teacher:references -s
rails g migration classroom type_classroom:references building:references department:references capacity:integer -s
...
rails g migration specific_request request_alternative:references specific_schedule:references -s

rails g migration report description:string{500} classroom:references  -s

rails g migration classroom_schedule classroom:references group:references cyclic_schedule:references -s
rails g migration opinion classroom_schedule:references student:references  -s

rails g migration classroom_event event:references specific_schedule:references classroom:references -s
